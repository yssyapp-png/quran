...
class BookmarkService {
  ...
  Future<void> toggleBookmark(Bookmark bookmark) async {
    ...
    final list = await getBookmarks();
    final exists = list
        .any((b) => b.suraNo == bookmark.suraNo && b.ayaNo == bookmark.ayaNo);
    if (exists) {
      list.removeWhere(
  ...
  Future<void> saveLastRead(int suraNo, int ayaNo, String suraName) async {
    ...
    await prefs.setString(_lastReadKey,
        jsonEncode({'suraNo': suraNo, 'ayaNo': ayaNo, 'suraName': suraName}));
  }
  ...
  Future<void> markPageRead(int page) async {
    ...
    pages.add(page);
    await prefs.setStringList(
        _readPagesKey, pages.map((e) => e.toString()).toList());
  }
